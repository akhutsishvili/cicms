...
class Core_m extends CI_Model {
	...
	elseif ($input_type == 'date') {
	    $t = ' <link rel="stylesheet" href="//code.jquery.com/ui/1.10.4/themes/smoothness/jquery-ui.css">
...
$( "#datepicker" ).datepicker({ dateFormat: "yy-mm-dd" });;
		...
		foreach ($value_arr as $val) {
		    ...
		    if ($q) {
			$tags .= '<div>' . anchor('cms/form/' . $table . '/' . $q['id'], $q[$relation_column], 'target="_blank" class="tags"') . '<span class="rm-csv" value="' . $q['id'] . '" col="' . $column . '">[X]</span></div>';
		    }
		    $items_from_relation_table = $this->db->from($relation)->order_by($relation_column, 'asc')->get()->result_array();
	...
	elseif ($input_type == 'file') {
		...
		$h .= "<div id='other-filters' class='badge'>მახასიათებლების განსაზღვრა</div>";
	    return $h;
	}
	// json
    }
